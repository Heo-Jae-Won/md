...
- 그 때는 deepFreeze 함수를 구현하든, lodash에서 쓰든 해줘야 한다.
```javascript
...
- Circle은 Object라는 부모를 갖게 된다. 그리고 prototype chaining으로 묶인다.
...
- 아래와 같이 기존의 code를 덮어씌우는 overriding이 일어날 수 있기 때문이다.
```javascript
function Person(name){
    this.name = name;
}
...
- extends를 넣어서 Prototype chaining을 걸 수도 있다.
```javascript
class Animal{
...
- 몇가지 다른 점은 동적인 상속 대상 결정이 가능하다는 점
- class만이 아니라 생성자 함수를 상속받을 수도 있다는 점이다.
...
- concise method를 쓰면 super도 활용이 가능하다.
```javascript
const Person = {
...
Object.setPrototypeOf(friend, Person); // prototype chaining 걸기. extends와 동일. setPrototypeOf(child, parent);
friend.greeting();
```
...
## <span style="color:#802548">_11.prototype method_</span>
- 아래는 array의 일반함수다.
```javascript
