...
- 요소노드의 static 상태는 attribute노드가 관리한다
- 요소노드의 live 상태는 DOM property가 관리한다.
...
<html>
    ...
    <script>
        ...
        $input.value = 'foo';
        console.log('value 어트리뷰트 값', $input.getAttribute('value')); // jaewon. foo로 나오지 않음.
    </script>
</html>
```

- 이러한 특성이 오류로 이어질 수 있다.
- 특히 getElementsByClassName(), getElementsByTagName()의 반환 값은 live 객체다.
- DOM property를 바꾸는 순간, 바로 반환 값 자체가 바뀌어버린다.
- 따라서 의도한 것은 className이 전부 txt-red가 되는 것인데, 실제로 그렇지 않게 되는 버그가 발생한다.
```html
<!DOCTYPE html>
<html>
  <head>
...
- 농협 setTimeout(fn,0);
- setTimeout을 사용하게 되면 callback 함수가 call stack으로 이동하는 시간 동안 js엔진이 작동하지 않는다.
