...
- 원시형은 객체의 property를 활용할 수 없다.
- 그러나 원시형의 경우, instance properties에 접근하는 경우, JS엔진이 객체로 wrapping해준다.
```javascript
...
- 또한 즉시실행함수로 만들면, 변수를 그냥 쓰는 것만으로도 함수가 호출된다.
```javascript
const counter = (function(){
   console.log('abc')
}());
...
- 그래도 똑같이 작동한다. 다른 점은 counter라는 객체의 property에 접근하는 게(counter.increase()) 아니라 함수로 호출한 뒤 다시 함수를 호출하는 것(counter().increase()) 뿐이다.
```javascript
const counter = function(){
    let num = 0;
    return {
...
- 변수만이 아니라 parameter도 기억된다.

- 아래는 parameter가 쓰인 예시다. 
```javascript
function add(num1) {
    ...
    <body>
        ...
        <script>
          ...
          $li.addEventListener('click', function({target}) {
            if(!target.matches('.list > li')){ //span은 무시
                  return;
            }
...
- step1이 가장 먼저 push되어 먼저 pop된다.
- step2는 setTimeout이라는 비동기 함수라서 timer로 이동한다.
- step2가 call stack에서 사라졌으니 step3가 pop된다.
- 0초가(실제론 14ms정도) 지난 step2의 callback함수가 event queue로 이동한다.
- step3가 call stack에서 사라지고 나서 callback함수가 call stack으로 이동한다.
- callback 함수인 console.log('step2')가 pop된다.

### <span style="color:#FFCCFF">setTimeout</span>

- rendering엔진은 JS엔진에 call stack에 함수가 있으면 작동하지 않는다.
...
- 로직이 오래 걸리는 함수가 있다고 해보자. 해당 함수 이후에 UI가 변화해야한다.
...
- 따라서 JS를 통해 text나 attribute가 변경된 DOM만 repaint가 이뤄진다. takesManyTime이후 변경되는 DOM들이 다시 rendering되어 화면이 출력된다.
```javascript
$(".btn").click(function() {
  showLoadingBar();
  takesManyTime();
...
- 이를 setTimeout을 이용해 loading bar를 띄워보자.
- 간단하게 아래와 같이 바꿀 수 있다.
...
for문을 돌면서 longTakingLogic()이 push된다. if문의 로직이 실행되어 DOM property가 변경된다.
하지만 곧바로 longTakingLogic()이 다시 push된다. JS엔진이 실행 중이라 if문의 로직이 실행되어 DOM property가 변경되어도 렌더링 엔진이 repaint할 수 없다. 
맨 마지막에 isCompleted가 false가 되면 for문을 빠져나오고 execute()가 종료되고 call stack이 빈다.
그럼 마지막 Dom property 조작만 rendering 엔진이 인식하여 repaint한다.
따라서 한꺼번에 쫙 차오르게 된다. 또한 실행중 버튼을 눌러도 call stack이 차있어 event queue에서 event handler인 alert함수가 대기하게 된다.
마찬가지로 call stack이 비게 되면 alert가 뜨게 된다. 따라서 repaint도, alert(실행중)도 execute()가 끝나기 전까지 작동하지 않는다.
```
...
function execute() {
    ...
    for (let i = 0; i < allCount; i++) {
      setTimeout(function(){
        longTakingLogic();
        remainCount = remainCount - 1;
        isCompleted = (i+1) % 1000 == 0; //10000으로 나누면 10%씩 증가. 1000으로 나누면 1%씩 증가
...
function execute() {
    ...
    for (let i = 0; i < allCount; i++) {
     ...
     /**/ console.log((allCount - remainCount) / allCount); //콘솔로그 실행시간으로 인해 UI update가 굉장히 느려짐
    }
  }
```

- setTimeout에서 주의할 점은 try ~ catch로 감싸면 안 된다는 점이다.
- setTimeout은 비동기 함수라서 콜백함수 호출되는 것을 기다리지 않고 종료된다.
- setTimeout의 callback 함수는 event queue로 push되어 call stack으로 브라우저에 의해 push된다.

- 따라서 callback 함수의 호출자가 없고, 콜백함수에서 난 error가 setTimeout()으로 전파되지 않고, error가 catch되지 않는다.
...
function getResponse() {
  request.get('https://jsonplaceholder.typicode.com/todos/1')
    ...
    .catch(err => console.error(err)); //오프라인 네트워크 장애, CORS 설정 등만 잡힘
}
getResponse();
```

- Promise를 동기처럼 처리하고 싶다면 async - await를 쓸 수도 있다.
- async await의 경우, try ~ catch문에서 error를 처리하는 경우가 흔하다. catch보다 가독성이 좋기 때문이라고 한다.
```javascript
const request = {
    get(url){
        return fetch(url);
