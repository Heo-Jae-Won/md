...
console.log(getName()) // ''. codepen에서 test하면 CodePen
```
...
- 아래와 같이 parameter가 들어올 type을 한정해줄 수 있다.
- 다만 입력 자체를 되돌리는 것은 아니다. 입력 이후 진행이 불가능하게 하는 것이다.
```javascript
...
```
html -> body -> ul -> li까지 모두 element Node다.
li라는 element Node는 총 3개다.
li[0]의 attribute node는 id="apple"이고, text node는 Apple이다.
li[1]의 attribute node는 id="banana"이고, text node는 banana이다.
li[2]의 attribute node는 id="orange"이고, text node는 orange이다.
```

- 아래는 node와 html의 간단한 예시다.
```html
<!DOCTYPE html>
<html>
    <body>
...
- live객체의 경우 주의하여 다룰 필요가 있다.
- 아래 예시를 살펴보자. 의도한 것은 className이 전부 txt-red가 되는 것인데, 실제로 그렇지 않게 되는 버그가 발생한다.
- DOM property를 바꾸는 순간, getElementsByClassName()의 반환 값 자체가 바뀌어버리기 때문이다.
...
- 아니면 배열로 만들어 live 객체가 아니게끔 바꿔 className을 바꾸면 된다.
```javascript
...
- 자식노드를 살리면서 요소를 추가하고 싶다면 insertadjascentHTML을 사용하자.
...
<button onclick="console.log('Hi');console.log('Lee');">click me!</button> //inline 함수
<button onclick="handleClick(event)">click me!</button>                    //event
...
```
- 실제 예시를 들어보자.
- data를 가져오는 데 오래 걸리는 함수가 있다고 해보자.
- 그래서 loading bar를 띄우게끔 했다.
- 하지만 예상과 다르게 loading bar가 떠오르지 않는다.
- showLoadingBar 함수의 실행이 끝나고 렌더링 엔진이 렌더링을 하려해도,태스크 큐에서 fetchManyData()가 실행되고 있다. 
- 렌더링 엔진은 JS엔진이 작동할 때 대기상태이므로 JS코드가 전부 실행될 때까지 기다리게 된다.
- 그럼 fetchManyData가 끝나고,  displayNoneLoadingBar가 끝나고, updateHTML가 끝난 뒤에야 rendering 엔진이 작동하며, 이 때는 loading bar가 이미 hide 되었다.
- 따라서 repaint할 loading bar 관련 DOM 요소가 없다. JS에 의해 영향을 받은 DOM요소가 있어야 렌더링 엔진이 render가 이뤄지는데, 해당 시점에서 loading bar DOM요소가 없다.
- 따라서 JS를 통해 text나 attribute가 변경된 DOM만 repaint가 이뤄진다. 즉 fetch로 받아온 data로 text를 변경하는 DOM들이 다시 rendering되어 화면이 출력된다.
```javascript
$(".btn").click(function() {
  showLoadingBar();
  fetchManyData();
...
- fetchManyData가 호출되고, fetch가 끝나면 displayNoneLoadingBar가 호출되며 마지막으로 updateHTML이 호출된다.
...
<html>
    ...
    var availableDataList = [];
		var targetList = ['deal1', 'deal23', 'deal37', 'deal4', 'deal590',
            ...
            'deal44', 'deal55'];
    var allCount = 100_000;
		var remainCount = allCount;

    //data 가져오는 것처럼 오래 걸리는 작업을 이중 for문으로 만듦
		function initData() {
			dataList = [];
