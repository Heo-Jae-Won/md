...
```
- 위에 썼던 예시에서 getCompanyName가 바로 method다.
```javascript
const obj = {
  name : 'company',
...
- 또 다른 방법으로는 apply, bind, call 등의 함수를 사용하는 것이다.
```javascript
...
HTML 요소는 렌더링 엔진에 의해 파싱돼 요소 노드 객체로 변환된다.
...
<html>
    <body>
        ...
        <script>
            const $elem = document.getElementById('banana'); //document 문서노드 접근 -> getElementsById() 요소 노드 접근
...
<html>
    ...
    <script>
        ...
        $input.value = 'foo';
        console.log('value 어트리뷰트 값', $input.getAttribute('value')); // car. foo로 나오지 않음.
    </script>
</html>
```
...
- [0]이 된 2번은 아래와 같은 방식으로는 접근이 불가능하다.
```html
<!DOCTYPE html>
<html>
  <head>
...
<html>
    ...
    <script>
        const $foo = document.getElementById('foo').getAttribute("mapId"); //$("#foo").attr("mapId");와 같이 Jquery에서 더 자주 쓰인다.
...
<button onclick="handleClick(this)">click me!</button>                     //attribute에서 직접 this를 사용하면 event의 target. button 자신
...
- 버블링을 막는 방법도 있다.
- stopPropagation()는 이벤트 버블링을 중단시켜 이벤트 위임 방식으로는 eventListener가 작동하지 않는다.
```html
<!DOCTYPE html>
<html>
    <body>
        ...
        <script>
            ...
            document.querySelector('.btn2').addEventListener('click',(e) => {
                e.stopPropagation();            // 버튼 중 btn2는 버블링 전파 불가.
...
- 또한 addeEventListener()만으로는 안되고 dispatchEvent()라는 함수를 호출해야 한다.
...
<body>
    ...
    <script>
        ...
        $button.dispatchEvent(customEvent); //그냥 addEventListerner 등록만으로는 안 됨. dispatch 호출해야함.
    </script>
</body>
</html>
```
...
event queue에 들어간 callback함수는 call stack이 비어있을 때 call stack으로 불려와 호출된다.
...
- step2는 setTimeout이라는 비동기 함수가 실행되고, callback fn이 timer로 이동한다.
...
```
- 로직이 오래 걸리는 함수인데, 해당 함수 이후에 UI가 변화해야한다.
- 기다리는 시간이 오래걸리므로 loading bar를 띄우게끔 했다.
- 하지만 예상과 다르게 loading bar가 떠오르지 않는다.
- showLoadingBar 함수의 실행이 끝나고 렌더링 엔진이 렌더링을 하려해도,태스크 큐에서 takesManyTime()가 실행되고 있다. 
- 렌더링 엔진은 JS엔진이 작동할 때 대기상태이므로 JS코드가 전부 실행될 때까지 기다리게 된다.

- 그럼 takesManyTime이 끝나고,  displayNoneLoadingBar가 끝나고, updateHTML가 끝난 뒤에야 rendering 엔진이 작동하며, 이 때는 loading bar가 이미 hide 되었다.
- 따라서 repaint할 loading bar 관련 DOM 요소가 없다. JS에 의해 영향을 받은 DOM요소가 있어야 렌더링 엔진이 render가 이뤄지는데, 해당 시점에서 loading bar DOM요소가 없다.
- 따라서 JS를 통해 text나 attribute가 변경된 DOM만 repaint가 이뤄진다. takesManyTime이후 변경되는 DOM들이 다시 rendering되어 화면이 출력된다. 거기엔 loading bar는 없다.

- setTimeout을 이용해 사라진 loading bar를 띄워보자.
- 간단하게 아래와 같이 바꿀 수 있다.
```javascript
$(".btn").click(function() {
  showLoadingBar();
...
```
- showLoadingBar가 호출된다.
- 그 이후 setTimeout이 호출되어 call stack이 잠시 비게 된다.
- 그동안 rendering 엔진이 UI를 repaint한다. 즉 loading bar가 나타난다.
- 렌더링이 끝나고 event queue에 있던 3개의 함수가 차례로 call stack에 push된다.

- takesManyTime가 호출되고, fetch가 끝나면 displayNoneLoadingBar가 호출되며 마지막으로 updateHTML이 호출된다.
- 따라서 loading bar가 사라지고, 바뀐 data로 repaint하여 HTML이 렌더링된다.
...
- 또한 percentage가 조금씩 올라가는 게 한번에 확 올라가버린다.
...
```
시작을 누르면, execute()가 call stack에 push된다.
execute 안에서 initData()가 call stack에 push된다.
for문을 돌면서 setTimeout()이 call stack에 push된다. 
setTimeout에 있는 callback function이 timer를 거쳐 event queue에 push된다.
그동안 call stack이 비어있다. 따라서 변경된 DOM property에 맞게 렌더링 엔진이 repaint한다. 
rendering 엔진이 repaint가 끝나면 JS 엔진이 나머지 for문을 다시 돈다.
이러한 과정이 반복된다.
```
...
const promiseGet = url => {
    return new Promise((resolve, reject) => {
        ...
        xhr.onload = () => { //event handler로 event queue에 callback 함수를 push한다.
            if(xhr.status === 200) {
...
- Promise에서는 catch에서 error를 처리하는 게 더 바람직하다.
- then은 성공했을 떄의 처리만 담당하는 것으로 한다.
```javascript
...
- fetch는 XMLHttpRequest와 기능이 같다. 다만 HTTP error code는 reject하지 않는다.
...
function getResponse() {
  request.get('https://jsonplaceholder.typicode.com/todos/1')
    .then(response => {
        if(!response.ok) {
            throw new Error(response.statusText); //fetch의 promise는 404, 500 등의 에러에 대해 에러를 reject하지 않고 ok를 false로 잡고 resolve하기 때문
        }

        return response.json();
    })
    .then(todos => console.log(todos))  // response.json()을 받은게 todos
    .catch(err => console.error(err)); //오프라인 네트워크 장애, CORS 설정 에러 등만 잡힘
}
getResponse();
```
...
async function getResponse() {
  try{
    const result = await request.get('https://jsonplaceholder.typicode.com/todos/1').then(res => res.json()).catch(err => err);
    console.log(result);
  } catch(e) {
    console.log(e);
  }
...
- 즉 getResponse()의 await가 붙은 함수가 fulfilled되어 response가 올 때까지 기다려주지 않는다.
```javascript
const request = {
    get(url){
        return fetch(url);
...
- 이미 resolve되어 Response instance가 되어 then()을 사용할 수가 없다.
```javascript
const request = {
    get(url){
        return fetch(url);
...
- then문을 나눴다면 이미 Response 객체가 되었으므로 Response라는 prototype에 있는 json()을 통해 결과값을 받아와야 한다.
...
- 비동기처리를 동기처럼 바꿔주기 위해 해당 함수에도 await를 붙여주면 값이 정상 출력된다.
```javascript
const request = {
    get(url){
        return fetch(url);
...
- 따라서 비동기 작업이 순차를 기다리지 않아도 되는 경우에는 async await가 오히려 독이 된다.
- 아래와 같애 Promise.all을 사용하는 것이 더 좋다.
```javascript
