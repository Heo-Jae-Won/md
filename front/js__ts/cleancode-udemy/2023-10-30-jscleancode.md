## <span style="color:#802548">_var와 const_</span>
- var를 사용하지 않는 이유는 hoisting과 function scope이기 때문이다.
...
- 재할당과 재선언을 반복해도 error가 나지 않는 것도 문제다.
...
- const는 재할당만 금지된다.
- 따라서 property 값들은 얼마든지 조작할 수 있다.
...
- type을 검사할 때는 prototype chain을 활용하는게 좋다.
...
## <span style="color:#802548">_type- undefined와 null_</span>
- 값이 명시적으로 없다면 null이다.
...
## <span style="color:#802548">_type- == 대신 ===쓰기_</span>
- ==은 값만 비교한다.
...
## <span style="color:#802548">_js의 숫자_</span>
...
## <span style="color:#802548">_경계값의 변수 네이밍_</span>
...
- 인간이 이해하기 쉬운 순서로 sequence에 맞춰 적는 것이 중요하다.
...
## <span style="color:#802548">_값식문_</span>
...
- if문을 사용하려면 어떻게 해야할까?
- 아래와 같이 즉시실행함수를 사용하여 바로 return해주면 값이 되기 떄문에 switch"문"으로도 가능하다.
...
## <span style="color:#802548">_if- 삼항연산자_</span>
...
## <span style="color:#802548">_if- 기본값주기_</span>
...
## <span style="color:#802548">_if- early return_</span>
- else문을 마구잡이로 쓰는 것을 지양해야 한다.
- early return으로 간단하게 바꿔준다.
...
## <span style="color:#802548">_if- 부정표현 쓰지말자_</span>
- 부정연산자는 최대한 덜 활용하자.
...
- 단 부정 조건문에서도 !로 전체를 괄호로 묶는 것은 바람직하지 않다.
- 각 조건들을 모두 따로 !로 붙여주는 게 가독성이 좋다.
...
## <span style="color:#802548">_if- lookup table_</span>
- 보통의 if문이다.
...
- 보통의 if문이 좀 가독성이 떨어진다면 아래처럼 switch case를 쓰자.
...
## <span style="color:#802548">_연산자 사용_</span>
...
- falsy가 아니라 null과 undefined일 때만 작동시키고 싶다면 ||가 아니라 ??(nullish)를 쓰자.
...
- 숫자 0과 관련된 다른 예시다. 숫자 0은 ??로 기본값을 준다.
...
console.log(helloWorld(0)); // 0은 falsy하기에 if문에서 false로 걸린다. Hello! World가 return된다.
...
console.log(helloWorld(0)); // Hello! 0; null과 undefined일 때만 바라보면 Hello! 0으로 출력된다.
```

- 논리연산자와 nullish연산자는 같이 쓸 수 없다.
- 값으로 변환한 뒤 다시 nullish 연산자를 써야 한다.
...
## <span style="color:#802548">_배열_</span>

- js에서 배열은 객체다. 그냥 0,1,2,3이라는 의미없는 key가 들어간 것과 똑같다고 생각할 수 있다
- 다만 배열의 index에는 문자열 key가 들어간다. js는 기본적으로 숫자 key를 허용하지 않는다.
- 대괄호 안에 숫자를 써서 가져오는 것도 실제로는 문자키로 취급된다.
...
- 하지만 Array prototype 안에만 있는 map, forEach 등은 사용이 불가능하다.
...
## <span style="color:#802548">_object- computed property_</span>
...
- computed property name의 다른 예시다.
- 잘 쓰이는 문법은 아니다.
...
## <span style="color:#802548">_object- destructuring_</span>
...
```
## <span style="color:#802548">_object- freeze_</span>
...
## <span style="color:#802548">_optional chaining_</span>
...
## <span style="color:#802548">_에러 다루기_</span>
...
## <span style="color:#802548">_HTML- semantic tag 지켜쓰기_</span>
```html
<html>
	<body>
...
## <span style="color:#802548">_HTML- innerHTML_</span>
...
## <span style="color:#802548">_HTML- data attribute_</span>
- 비표준 속성을 쓰지 않게 생긴 표준
...
## <span style="color:#802548">_HTML- black box eventListener_</span>
...
## <span style="color:#802548">_로직 문단 사이에 공백을 잘 쓰자_</span>
...
## <span style="color:#802548">_중첩 줄이기_</span>
- early return
...
## <span style="color:#802548">_function- pure function_</span>
...
## <span style="color:#802548">_function- parameter 기본값 설정_</span>
...
## <span style="color:#802548">_function- parameter 줄이기_</span>
...
function createCar({brand,name,color,type}){
	var name = options.name;
	var brand = options.brand;
	var color = options.color;
	var type = options.type;
...
- 화살표함수 내에서 this를 사용한다면 concise method를 써야한다.
...
- 화살표함수에서는 arguments를 쓸 수 없다.
- rest parameter를 활용해야한다.
...
## <span style="color:#802548">_function- callback function_</span>
...
## <span style="color:#802548">_function- 클로저_</span>
