...
- but what about using index-organized table?
...
## <span style="color:#802548">_many vertical scan vs many horizental scan: betwenn in-list_</span>
...
- 아래와 같은 쿼리가 있는데, 인덱스가 (회사코드,지역코드,commodity_name)으로 구성됐다고 해보자.
...
- or 조건의 경우, 아래와 같은 조건이 아니면 쓰지 않는 게 효율적이다.
  ...
  - 다만 or expansion으로 활용할 수 있다면 상관없다.
    - 예를 들어, (customer_id, 타입, transaction_date)를 index로 묶어보자.
    - customer_id라는 선두컬럼만 범위조건이 아니면 or expansion이 발동할 수 있다.
...
SELECT *
FROM transaction
WHERE (customer_id is null or customer_id = :cust_id)
...
SELECT *
FROM transaction
WHERE customer_id LIKE :cust_id || '%'
AND transaction_date BETWEEN :dt1 AND :dt2
```
...
- 숫자형 컬럼에도 LIKE는 안된다.
  ...
  - 묵시적 형변환으로 인해 (customer_id, transaction_date) index는 아예 무력화된다는 것도 문제다.
```sql
/**customer_id는 int */
SELECT * FROM transaction
WHERE transaction_date = :trd_dt
AND customer_id/**실제론 TO_CHAR(customer_id)로 변환됨 */ LIKE :cust_id || '%'
```
...
```sql
SELECT customer_no, customer_name, 거주지역
FROM customer
WHERE 거주지역 = '서울'
AND 혈액형 IN ('A','O')
ORDER BY 연령
...
- 위에서 고개등급, customer_no, transaction_date는 필수 조건이다. transaction유형과 commodity_no는 옵션조건이다.
  - customer등급, customer_no는 어떤 컬럼이 앞에 오든 index range scan에 영향이 없다. 둘 모두 = 조건이기 때문이다.
  - transaction유형과 commodity_no도 어떤 컬럼이 앞에 오든 index range scan에 영향이 없다.
...
```sql
SELECT e.사원명, c.customer_name, c.전화번호
...
SELECT /*+ ordered use_nl(c) index(e) index(c) */ 
  e.사원번호, e.사원명, e.입사일자, c.customer_no, c.customer_name, c.전화번호, c.최종주문금액
FROM 사원 e, customer c
...
SELECT /*+ ordered use_merge(c) */
      e.사원번호, e.사원명, e.입사일자
      , c.customer_no, c.customer_name, c.전화번호, c.최종주문금액
FROM 사원 e, customer c
WHERE c.관리사원번호 = e.사원번호
AND e.입사일자 >= '19960101'
AND e.부서코드 = 'Z123'
...
```sql
SELECT customer_no, customer_name, 전화번호, 최종주문금액, 관리사원번호
FROM customer c
WHERE 최종주문금액 >= 20000
ORDER BY 관리사원번호
```
...
```sql
SELECT c.계약번호, c.상품코드, p.commodity_name, p.상품구분코드
FROM 계약 c, 상품 p
...
- 이를 NL join으로 바꿔준다.
```sql
SELECT /*+ leading(c) use_nl(p) */ c.계약번호, c.상품코드, p.commodity_name, p.상품구분코드
FROM 계약 c, 상품 p
WHERE c.지점ID = :brch_id
AND p.상품코드 = c.상품코드
ORDER BY c.계얄일시 DESC
