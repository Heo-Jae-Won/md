...
- so if u decide which index should be leading column and which columns should be included, this can be standard
    ...
    - table access minimalization
        ...
        - 물론 특정 컬럼, customer_no 기준으로 CF계수가 높다면, 다른 컬럼, 진료일자 기준으로는 CF계수가 낮을 것이다.
...
- let's suppose that PK is consisted of (field_type_code,field_code,base_date)
...
## <span style="color:#802548">_index good practice condsidering type priority_</span>
...
- 시작점을 한정지을 수 있기 때문이다.
```sql
select 렌탈관리번호, customer명, 서비스관리번호, 서비스번호, 예약접수일시
...
```sql
SELECT /*+ INDEX_SS(t 월별customer별판매집계_IDX2) */ count(*)
FROM 월별customer별판매집계 t
WHERE 판매구분 = 'A'
...
```sql
SELECT /*+ num_index_keys(a customer별가입상품_X1 1) */ *
FROM customer별가입상품 a
...
```sql
SELECT /*+ num_index_keys(a customer별가입상품_X1 2) */ *
FROM customer별가입상품 a
WHERE customer_no = :cust_no
AND 상품ID in ('NH00037', 'NH00041','NH00050')
```
...
- 즉 LIKE로 사실상 쓴 셈이다. 
```sql
SELECT transaction_date, 종목코드, 투자자유형코드
FROM 일별종목transaction
WHERE transaction_date BETWEEN :시작일자 AND :종료일자               /**필수조건 */
...
- or 조건의 경우, 아래와 같은 조건이 아니면 쓰지 않는 게 효율적이다.
  ...
  - 다만 or expansion으로 활용할 수 있다면 상관없다.
    - 예를 들어, (customerID, 타입, transaction_date)를 index로 묶어보자.
    - customerID라는 선두컬럼만 범위조건이 아니면 or expansion이 발동할 수 있다.
...
AND (
  (:dt_type = 'A' AND transaction_date BETWEEN :dt1 and :dt2)
  or
...
SELECT *
FROM transaction
WHERE (customerID is null or customerID = :cust_id)
AND transaction_date BETWEEN :dt1 and :dt2
```
...
- 숫자형 컬럼에도 LIKE는 안된다.
  ...
  - 묵시적 형변환으로 인해 (customerID, transaction_date) index는 아예 무력화된다는 것도 문제다.
```sql
/**customerID는 int */
SELECT * FROM transaction
WHERE transaction_date = :trd_dt
AND customerID/**실제론 TO_CHAR(customerID)로 변환됨 */ LIKE :cust_id || '%'
```

- 옵션 조건은 union all을 통해 효율적인 index scan을 할 수도 있다.
  - 단, 이경우 (cust_id, transaction_date)와 (transaction_date)라는 index가 모두 존재해야 한다.
...
SELECT *
FROM transaction
WHERE customerID = nvl(:cust_id, customerID)
AND transaction_date BETWEEN :dt1 and :dt2
```
...
```sql
SELECT customer_no, customer명, 거주지역
FROM customer
WHERE 거주지역 = '서울'
AND 혈액형 IN ('A','O')
ORDER BY 연령
...
- 예를 들어, 성별은 선택도가 높고 customer_no는 선택도가 매우 낮다.
- 그렇지만 성별이 조건절 앞에 오든 뒤에 오든 index scan범위는 똑같다.
```sql
WHERE 성별   = 'M'
AND customer_no = 'DDDE'
```
...
- 위에서 고개등급, customer_no, transaction_date는 필수 조건이다. transaction유형과 상품번호는 옵션조건이다.
  - customer등급, customer_no는 어떤 컬럼이 앞에 오든 index range scan에 영향이 없다. 둘 모두 = 조건이기 때문이다.
  - transaction유형과 상품번호도 어떤 컬럼이 앞에 오든 index range scan에 영향이 없다.
    - index scan범위가 customer등급, customer_no, transaction_date에 의해 결정되기 때문이다. 
...
- 중복제거를 실습해보자.
```
PK: transaction_date, 관리지점번호, 일련번호
N1: account_no, transaction_date
N2: 결제일자, 관리지점번호
N3: transaction_date, 종목코드
N4: transaction_date, account_no
```

- transaction_date가 늘 부등호라면, 종목코드와 account_no는 index filter조건으로 고정이다.
- 아래와 같이 줄여준다.
```
PK: transaction_date, 관리지점번호, 일련번호
N1: account_no, transaction_date
N2: 결제일자, 관리지점번호
N3: transaction_date, 종목코드, account_no
```
...
```sql
SELECT e.사원명, c.customer명, c.전화번호
FROM 사원 e, customer c
WHERE e.입사일자 >= '19960101'
AND c.관리사원번호 = e.사원번호
```

- 위와 같은 경우, outer인 사원의 입사일자는 1번 조회다.
- 그렇게 scan해온(index이든 table이든) record를 inner인 customer의 관리사원번호 조건에 맞게 다시 record를 scan한다.
...
SELECT /*+ ordered use_nl(c) index(e) index(c) */ 
  e.사원번호, e.사원명, e.입사일자, c.customer_no, c.customer명, c.전화번호, c.최종주문금액
FROM 사원 e, customer c
...
3. 조인 조건인 관리사원번호를 만족하는 index record를 찾기 위해 customer_X1 index를 range scan
4. customer_X1 index에서 읽은 ROWID로 customer 테이블을 access해 테이블 filter 조건으로 조건절 4번 사용
```
...
- 사원 테이블에서 얻은 record가 3개인데, 사원의 사원번호가 10, 12, 15라고 해보자.
  - 그럼 c.관리사원번호가 10인 경우에 최종주문금액이 20,000이 넘는 customer의 record를 가져온다.
  - c.관리사원번호가 12인 경우에 최종주문금액이 20,000이 넘는 customer의 record를 가져온다.
  - c.관리사원번호가 15인 경우에 최종주문금액이 20,000이 넘는 customer의 record를 가져온다.
- 이렇게 outer에서 가져온 각 record에 맞는 customer의 record를 모두 종합하면 그게 NL join의 결과물이다.
...
- 위의 그림에서 튜닝포인트는 4가지다.
  ...
  - 사원 table에서 조건을 만족하는 filtering된 record와 결합하는 customer_X1 index와 결합하는 join횟수
    - 부서코드 Z=123을 만족하는 건수가 3개였기에, 해당 갯수만큼 조인시도를 했다.
    - 조인을 위해 customer_X1 index를 탐색하는 range scan 횟수가 많을 수록 성능이 안 좋아진다.
  - customer_X1 index를 읽고 customer 테이블 access하는 횟수
...
- 그러나 2780개를 가지고 가서 customer table과 join한 결과는 겨우 5 row다. 매우 비효율적인 join인 셈이다.
...
SELECT /*+ ordered use_merge(c) */
      e.사원번호, e.사원명, e.입사일자
      , c.customer_no, c.customer명, c.전화번호, c.최종주문금액
FROM 사원 e, customer c
WHERE c.관리사원번호 = e.사원번호
AND e.입사일자 >= '19960101'
AND e.부서코드 = 'Z123'
...
```sql
SELECT customer_no, customer명, 전화번호, 최종주문금액, 관리사원번호
FROM customer c
WHERE 최종주문금액 >= 20000
ORDER BY 관리사원번호
```

- PGA에 저장한 사원 데이터를 스캔하면서 PGA에 저장한 customer 데이터와 join한다.
```sql
begin
...
- 위와 같이 sort merge join은 사원 데이터를 기준으로 매번 customer 데이터를 full scan하진 않는다.
  - customer 데이터가 정렬돼있으므로 join record가 시작되는 지점을 쉽게 찾을 수 있다.
...
- 아래 조건에 해당하는 customer에티러를 읽어 앞서 생성한 해시 테이블을 탐색한다.
```sql
SELECT customer_no, customer명, 전화번호, 최종주문금액, 관리사원번호
FROM customer
WHERE 최종주문금액 >= 20000
...
```sql
SELECT c.customer_no, c.customer명
FROM customer c
...
```sql
SELECT /*+ leading(transaction@subq) use_nl(c) */ c.customer_no, c.customer명
FROM customer c
WHERE c.가입일시 >= trunc(add_months(sysdate, -1), 'mm')
AND EXISTS (
...
```sql
SELECT c.customer_no, c.customer명, t.평균transaction, t.최소transaction, t.최대거리
FROM customer c,
...
```sql
SELECT c.customer_no, c.customer명, t.평균transaction, t.최소transaction, t.최대transaction
FROM customer c,
...
- join에 쓰이는 데이터를 캐시에서 찾으면 join 성능이 매우 좋아진다.
  ...
  - 하지만 PGA에 캐시를 할당하는 것이라, 캐시에 담을 join access에 쓰이는 record가 적어야 한다.
    ...
    - 만약 customer이 100만명인데, customerID로 join access를 쓴다면 100만개를 캐시에 저장해야 한다.
...
SELECT transaction번호, customer_no, 영업조직ID, transaction구분코드,
        (SELECT customer명 FROM customer WHERE customer_no = t.customer_no) customer명
FROM transaction t
...
## <span style="color:#802548">_sort by가 필요하다면 sortByte를 적게_</span>
...
```sql
SELECT LPAD(상품번호, 30) || LPAD(상품명, 30) || LPAD(customerID, 10)
        ||  LPAD(customer명, 20) || TO_CHAR(주문일시, 'yyyymmdd hh24:mi:ss')
FROM 주문상품
...
SELECT LPAD(상품번호, 30) || LPAD(상품명, 30) || LPAD(customerID, 10)
        ||  LPAD(customer명, 20) || TO_CHAR(주문일시, 'yyyymmdd hh24:mi:ss')
FROM (
  SELECT 상품번호, 상품명, customerID, customer명, 주문일시
  FROM 주문상품
  WHERE 주문일시 BETWEEN :start and :end
  ORDER BY 상품번호
)
...
FROM (
  SELECT ROWNUM no, a.*
  FROM (
    SELECT transaction일시, success_transaction, success_transaction_count, transaction_amount
    FROM 종목transaction
    WHERE 종목코드 = 'KR123456'
    AND transaction일시 >= '20180304'
    ORDER BY transaction일시
  ) a
...
## <span style="color:#802548">_group by에 index 적용_</span>
- 그룹핑 연산에서도 index를 활용할 수 있다.
```sql
...
CREATE INNDEX 주문_X04 on 주문 (customerID, 배송일자);
```
...
- 만약 최종변경 일시를 관리한다면 아래와 같이 최고변경일시만 검색해주면 된다.
...
UPDATE customer SET 적립포인트 = :적립포인트, 변경일시 = SYSDATE
WHERE customer_no = :cust_num
AND 변경일시 = :mod_dt;
...
```sql
INSERT INTO 주문(상품코드, customerID, 주문일시, 상점번호, ...)
values(#{상품코드}, #{customerID}, #{주문일시}, #{상점번호}, ...)
```
...
```sql
INSERT INTO 주문
SELECT :상품코드, :customerID, :주문일시, :상점번호, ...
FROM 상품
WHERE 상품코드 = :상품코드
