...
## <span style="color:#802548">_index sort의 꽃 -top N_</span>
TODO
...
- Top N 쿼리를 하면 INDEX RECORD를 일정 갯수 이상 읽는 순간 바로 멈춘다.
  - (종목코드, 거래일시)로 index를 구성하면 열개 table data 레코드를 읽는 순간 멈춘다는 의미다.
  - 해당 조건을 만족하는 더 많은 값들을 탐색하지 않는다는 의미다. random access 면에서 효율적으로 작동하게 된다.
```sql
SELECT * FROM (
  SELECT 거래일시, 체결건수, 체결수량, 거래대금
  FROM 종목거래
...
- top n은 max, min 연산을 대체하는 데도 사용할 수 있다.
  - 특히 TOP N 쿼리는 index 중 하나가 빠져도 작동해서 유연하다.
