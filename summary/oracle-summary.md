...
- 다만 table scan과 달리 index scan은 rowid를 통해 다른 데이터block을 순차로 읽지 않고 곧바로 접근할 수 있을 뿐이다.
- table은 순차적으로 모든 데이터블록을 읽는 점에서 sequential access라고 한다.
- 반면에 index는 직접 필요한 데이터 블록에 접근할 수 있기 때문에 random(임의) access라고 한다.
...
- 그래서 direct path I/O를 제외한 모든 blcok I/O는 메모리 버퍼캐시를 경유한다.
...
- 그러나 random access는 직접 교실을 찾아가며 방문하는 것과 같기 때문이다.
...
- index range scan양이 똑같기 때문이다. block I/O call할 양이 똑같다는 의미다.
...
- 수행빈도
- DML 영향
## <span style="color:#802548">_옵션처리_</span>
...
## <span style="color:#802548">_부분범위처리_</span>

## <span style="color:#802548">_execution plan_</span>

## <span style="color:#802548">_trace파일 만들기_</span>
